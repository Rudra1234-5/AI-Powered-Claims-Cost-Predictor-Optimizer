...
# AI-Powered Prediction Section
    ...
    if prediction_option == "Forecast Data using Prophet":
        ...
        if st.button("Ask") and user_question:
            try:
                context = f"You are a helpful healthcare analyst. Here's a healthcare dataset summary:\n\n{df.head().to_string()}. If asked for future Data Forecast using Prophet, use from Prophet import prophet. Use the file path for the csv as Gen_AI_sample_data csv. For questions like 'What is the predicted peak month for allowed amount?', calculate and return the peak month based on forecasted data. Ensure that the output includes both the numerical prediction and a graph."
                ...
                response = client.chat.completions.create(
                    ...
                    max_tokens=500  # Limit the token count for more consistent output
                )
                
                content = response.choices[0].message.content
                st.write(content)

                # Handling specific question like "predicted peak month"
                if "predicted peak month" in user_question.lower():
                    # Perform the forecast calculation to get the predicted peak month and cost
                    df_prophet = df[["service_year_month", "paid_amount"]].rename(columns={"service_year_month": "ds", "paid_amount": "y"})
                    model = Prophet()
                    model.fit(df_prophet)
                    future = model.make_future_dataframe(periods=12, freq='M')
                    forecast = model.predict(future)

                    # Extract the predicted peak month (max yhat value)
                    peak_month = forecast.loc[forecast["yhat"].idxmax(), "ds"]
                    st.markdown(f"### Predicted Peak Month: {peak_month.strftime('%B %Y')}")
